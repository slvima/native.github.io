...
function process_form($form) {
	if ($_SERVER['REQUEST_METHOD'] != 'POST')
		die(get_form_error_response('Неизвестный метод запроса сервера'));

	if (formthrottle_too_many_submissions($_SERVER['REMOTE_ADDR']))
		die(get_form_error_response('Недопустимо высокое количество отправок с этого IP-адреса за последнее время'));
	...
	switch($properties['type']) {
		...
		case 'recaptcha':
			if (!array_key_exists('recaptcha', $form) || !array_key_exists('private_key', $form['recaptcha']) || empty($form['recaptcha']['private_key']))
				die(get_form_error_response('Недействительный закрытый ключ reCAPTCHA.'));
		...
		default:
			die(get_form_error_response(sprintf('Неизвестный тип поля \'%s\'.', $properties['type'])));
	}
}
...
function email_form_submission($form) {
	...
	if(!$sent)
		die(get_form_error_response('Не удалось отправить сообщение эл. почты'));
...
function get_email_body($subject, $heading, $fields) {
	...
	$message .= '</table>';
	$message .= '<br/><br/>';
	$message .= '<div style="background-color: #eeeeee; font-size: 10px; line-height: 11px;">Формы, отправленные с веб-сайта: ' . encode_for_form($_SERVER['SERVER_NAME']) . '</div>';
	$message .= '<div style="background-color: #eeeeee; font-size: 10px; line-height: 11px;">IP-адрес посетителя: ' . encode_for_form($_SERVER['REMOTE_ADDR']) . '</div>';
	$message .= '</body></html>';
...
function get_form_field_value($field, $properties) {
	...
	switch($properties['type']) {
		case 'checkbox':
			return (($value == '1' || $value == 'true') ? 'Отмечено' : 'Флажок не установлен');
